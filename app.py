...
app.config['MAIL_USERNAME'] = 'ojasrkale@gmail.com'
app.config['MAIL_PASSWORD'] = 'yivb ruzx pawb xfqp'
...
path = 'Face-Recognition-Based-Employee-Attendance-Logger-main/static/TrainingImages'
...
def delete(id):
    ...
    df = pd.read_csv("Face-Recognition-Based-Employee-Attendance-Logger-main/static/records.csv")
    df.loc[df["Id"] == id, "Status"] = "Terminated"
    df.to_csv("Face-Recognition-Based-Employee-Attendance-Logger-main/static/records.csv", index=False)
        ...
        if ret:
            ...
            for faceLoc in facesLoc:  
                ...
                if result['dominant_emotion'] == 'happy' and timeit.default_timer() - start > 5:
                    ...
                    if flag:
                        cv2.putText(frame, str(num), (150, 200),cv2.FONT_HERSHEY_SIMPLEX, 8, (255, 255, 255), 20)
                        time.sleep(1)
                        num = num-1
...
def gen_frames():
    ...
    def markEntry(id):
        with open("Face-Recognition-Based-Employee-Attendance-Logger-main/static/records.csv", 'r+') as f:
...
def AttendanceSheet():
    rows = []
    with open("Face-Recognition-Based-Employee-Attendance-Logger-main/static/records.csv") as f:
...
def downloadToday():
    ...
    df.to_csv("Face-Recognition-Based-Employee-Attendance-Logger-main/static/todayAttendance.csv", index=False)
...
def helpBot():
    ...
    with open('Face-Recognition-Based-Employee-Attendance-Logger-main/static/help.json') as f:
...
def gen_frames1():  
    ...
    while True:
        ...
        # Ensure the face crop has valid dimensions
        if face_crop.size == 0:
            print("Error: Face crop has zero size.")
            continue
            
        # Perform emotion detection with enforce_detection set to False
        try:
        ...
        cv2.putText(frame, emotion, (startX, startY - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
...
def genframes2():
    ...
    dlib_facelandmark = dlib.shape_predictor("D:\Downloads\Face-Recognition-Based-Employee-Attendance-Logger-main\Face-Recognition-Based-Employee-Attendance-Logger-main\shape_predictor_68_face_landmarks.dat")
